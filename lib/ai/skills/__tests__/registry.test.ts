...
describe('SkillRegistry — getSkill', () => {
  ...
  it('returns full definition for performance-analysis', () => {
    const skill = registry.getSkill('performance-analysis')
    expect(skill).not.toBeNull()
    expect(skill!.id).toBe('performance-analysis')
    expect(skill!.name).toBe('Performance Analysis')
  })

  it('returns full definition for code-complexity', () => {
    const skill = registry.getSkill('code-complexity')
    expect(skill).not.toBeNull()
    expect(skill!.id).toBe('code-complexity')
    expect(skill!.name).toBe('Code Complexity')
  })

  it('returns full definition for testing-quality', () => {
    const skill = registry.getSkill('testing-quality')
    expect(skill).not.toBeNull()
    expect(skill!.id).toBe('testing-quality')
    expect(skill!.name).toBe('Testing Quality')
  })

  it('returns full definition for api-design-review', () => {
    const skill = registry.getSkill('api-design-review')
    expect(skill).not.toBeNull()
    expect(skill!.id).toBe('api-design-review')
  ...
  it('returns full definition for database-query-review', () => {
    const skill = registry.getSkill('database-query-review')
    expect(skill).not.toBeNull()
    expect(skill!.id).toBe('database-query-review')
    expect(skill!.name).toBe('Database Query Review')
  })

  it('returns full definition for migration-planning', () => {
    const skill = registry.getSkill('migration-planning')
    expect(skill).not.toBeNull()
    expect(skill!.id).toBe('migration-planning')
    expect(skill!.name).toBe('Migration Planning')
  })

  it('returns full definition for state-management-review', () => {
    const skill = registry.getSkill('state-management-review')
    expect(skill).not.toBeNull()
    expect(skill!.id).toBe('state-management-review')
    expect(skill!.name).toBe('State Management Review')
  })

  it('returns full definition for type-safety-audit', () => {
    const skill = registry.getSkill('type-safety-audit')
    expect(skill).not.toBeNull()
